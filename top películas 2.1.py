...
#Definimos la ubicación del csv.
...
# Leemos las películas desde el csv y las agregamos a la lista películas.
peliculas = []
...
#Devuelve el top 10 según el criterio de puntaje y, en segundo lugar, la duración.
def calcular_top10():
    top_totales = sorted(peliculas, key=lambda x: (x.puntaje, x.duracion), reverse=True)
    return top_totales[:10]
...
def promediar_duracion():
    suma = 0
    for p in calcular_top10():
        suma += p.duracion
    promedio_duracion = int(suma / len(calcular_top10()))
...
def promediar_antiguedad():
    restos_año = 0
    for p in calcular_top10():
        restos_año += (anio_actual - p.año)
    promedio_antiguedad = int(restos_año / len(calcular_top10()))
